...
if [ "$ITEM" = "running-slave" ]; then
    ...
    RES=`/usr/local/mysql/bin/mysql -uzabbix -pxxx -S /tmp/mysql.sock -e 'SHOW SLAVE STATUS\G' 2>/dev/null | egrep '(Slave_IO_Running|Slave_SQL_Running):' | awk -F: '{print $2}' | tr '\n' ','`
    if [ "$RES" = " Yes, Yes," ]; then
        echo 1
    else
        echo 0
